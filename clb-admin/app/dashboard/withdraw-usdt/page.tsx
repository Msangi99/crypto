...
export default function WithdrawUsdtPage() {
  ...
  const load = useCallback(async (currentPage = 1) => {
    ...
    try {
      ...
      const res = await api.getAdminWithdrawals(currentPage, limit, status, "USDT");
      setWithdrawals(res.withdrawals || []);
      setTotal(res.total || 0);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed to load USDT withdrawals");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast.success("USDT withdrawal approved");
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast.success("USDT withdrawal rejected");
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Withdraw USDT</h1>
            <p className="text-sm text-[#999] mt-0.5">
              Manage USDT withdrawal requests from app users.
              {pendingCount > 0 && (
      ...
      <Card className="bg-[#111] border-[#2A2A2A]">
        <CardHeader className="pb-3">
          <CardTitle className="text-white text-lg flex items-center gap-2">
            ...
            <Badge variant="outline" className="border-emerald-500/40 text-emerald-400 bg-emerald-500/10 text-xs font-normal">
              {total} total
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-16 text-[#666]">No USDT withdrawal requests found.</div>
          ) : (
                  ...
                  {withdrawals.map((w) => (
                    <tr key={w.id} className="border-b border-[#1A1A1A] hover:bg-[#1A1A1A]/50">
                      ...
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-1">
                          <code className="text-xs text-[#ccc] bg-[#1A1A1A] px-1.5 py-0.5 rounded">
                            {shortAddr(w.user.walletAddress)}
                          </code>
                          <button
                            onClick={() => copyText(w.user.walletAddress)}
                      ...
                      <td className="py-3 px-2 text-right text-white font-mono">
                        {w.amount.toFixed(2)} <span className="text-emerald-400 text-xs">USDT</span>
                      </td>
      ...
      <Dialog open={!!approveDialog} onOpenChange={(open) => !open && setApproveDialog(null)}>
        <DialogContent className="bg-[#111] border-[#2A2A2A] text-white max-w-lg">
          <DialogHeader>
            <DialogTitle>Approve USDT Withdrawal</DialogTitle>
          </DialogHeader>
          {approveDialog && (
            <div className="space-y-4">
              <div className="bg-[#1A1A1A] rounded-lg p-3 border border-[#2A2A2A]">
                <div className="text-[#666] text-xs mb-1">Requested by</div>
                <div className="text-white text-sm font-medium">
                  {approveDialog.user.username || "Anonymous"}
                  {approveDialog.user.email && (
                    <span className="text-[#666] font-normal ml-2">({approveDialog.user.email})</span>
                  )}
                </div>
                <div className="flex items-center gap-1 mt-1">
                  <code className="text-xs text-[#999]">{shortAddr(approveDialog.user.walletAddress)}</code>
                  <button onClick={() => copyText(approveDialog.user.walletAddress)} className="p-0.5 hover:bg-[#2A2A2A] rounded">
              ...
              <p className="text-sm text-[#999]">
                Send <strong className="text-white">{(approveDialog.amount - approveDialog.fee).toFixed(2)} USDT</strong> to the address below, then paste the transaction hash and confirm.
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="bg-[#1A1A1A] rounded-lg p-3 border border-[#2A2A2A]">
                  ...
                  <div className="text-white font-mono font-semibold">{approveDialog.amount.toFixed(2)} USDT</div>
                </div>
                <div className="bg-[#1A1A1A] rounded-lg p-3 border border-[#2A2A2A]">
                  <div className="text-[#666] text-xs mb-1">Net (send this)</div>
                  <div className="text-emerald-400 font-mono font-semibold">{(approveDialog.amount - approveDialog.fee).toFixed(2)} USDT</div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-[#666]">
                  Paste the txHash after you send the USDT. You can leave this empty and add it later.
                </p>
              </div>
            </div>
          )}
      ...
      <Dialog open={!!rejectDialog} onOpenChange={(open) => !open && setRejectDialog(null)}>
        <DialogContent className="bg-[#111] border-[#2A2A2A] text-white max-w-md">
          <DialogHeader>
            <DialogTitle>Reject USDT Withdrawal</DialogTitle>
          </DialogHeader>
          {rejectDialog && (
            <div className="space-y-4">
              <p className="text-sm text-[#999]">
                Reject <strong className="text-white">{rejectDialog.amount.toFixed(2)} USDT</strong> withdrawal
